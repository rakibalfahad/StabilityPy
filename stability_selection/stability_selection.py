...
stability selection with GPU acceleration and parallel processing.
...
def _fit_bootstrap_sample(base_estimator, X, y, lambda_name, lambda_value,
    ...
    """
    # Try to set GPU for the estimator if available
    if use_gpu and hasattr(base_estimator, 'use_gpu'):
        base_estimator.set_params(use_gpu=True)
...
class StabilitySelection(BaseEstimator, TransformerMixin):
    ...
    batch_size : int, default=None
        Batch size for GPU processing. If None, processes all data at once.
    ...
    def fit(self, X, y):
        ...
        for idx, lambda_value in lambda_iter:
            ...
            selected_variables = Parallel(
                n_jobs=self.n_jobs, verbose=0 if use_tqdm else self.verbose,
            ...
            # Calculate mean selection frequencies
    ...
    def transform(self, X, threshold=None):
        ...
        if not mask.any():
            warnings.warn("No features were selected: either the data is"
